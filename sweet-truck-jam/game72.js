...
const FEED_VISIBLE_MARGIN=SWEET_RADIUS*1.5;
...
function garageFootprintPoly(g,pad=0){
  if(!g)return null;
...
function canDriveOut(t,trucks,explicitGarage=null){
...
function truckPositionLegal(t,x,y,trucks,explicitGarage=null){
...
function addUndergroundGarage(gen,r,n){
  ...
  const extra=n<4?2:n<7?rint(r,3,4):n<12?rint(r,4,5):rint(r,5,7),queue=[];
...
function mixSweetRowColours(rows,r){
...
function finishGeneratedCluster(trucks,order,R,n){
  ...
  gen.sweetRows=sweet.rows;gen.difficulty={maxParked:sweet.result.maxParked,forcedWrong:sweet.result.forcedWrong,initialFree:trucks.filter(t=>canDriveOut(t,trucks,gen.garage)).length};
...
function challengeGeneratedCluster(trucks,order,R,n){
  ...
  assignChallengeColors(gen,R,n);
  const sweet=buildHardSweetRows(gen,R,n);if(!sweet)return null;
  const minParked=n>=8?4:3,minWrong=n>=8?3:2;
...
function safeGeneratedCluster(trucks,order,R,n){
...
function fallbackLevel(n){
  ...
  for(let attempt=0;attempt<180;attempt++){
    ...
    const clear=initialClearCount(trucks);if(clear<1||clear>8)continue;
    const order=removalOrder(trucks,R);if(!order)continue;
...
function start(n){
  ...
  document.body.dataset.safeFallback=dd.safeFallback?'1':'0';
...
function drawBoosters(){
  ...
  const y=844,buttons=[{x:70,label:'AUTO',sub:state.boosters.auto},{x:165,label:fifthOpen?'5TH OPEN':'+ SLOT',sub:fifthOpen?'✓':'100'},{x:260,label:'SHUFFLE',sub:state.boosters.shuffle},{x:355,label:'SHOP',sub:'◈'}];
...
function drawFeederRows(feed,side,dt){
  ...
  for(let r=rowCount-1;r>=0;r--){
    ...
    // One shared visual-row position for all four sweets. Draw every row whose
...
function drawGarage(){
  ...
  roundedRect(-g.length/2,-g.width/2,g.length,g.width,7,'#2d3540','#f3c64c',2);
...
function advanceLoopOneRow(){
  ...
  if(!isGap){
    ...
    if(slotIndex>=0){
      ...
      // The sweet physically closest to the chosen truck peels off first.
...
function frame(ts){const dt=Math.min(.10,(ts-last)/1000||.016);last=ts;update(dt);drawBackground();drawQueue(dt);drawSlotsAndParked();drawGarage();drawYard();drawMotions();drawParticles();drawTopUI();drawBoosters();if(document.body.dataset.gameReady!=='1')document.body.dataset.gameReady='1';requestAnimationFrame(frame)}

function showStartupFailure(err,requestedLevel){
